...
class StatementAnalyzer
{
            ...
            extends AstVisitor<Scope, Optional<Scope>>
    {
        ...
        protected Scope visitTableFunctionInvocation(TableFunctionInvocation node, Optional<Scope> scope)
        {
            ...
            List<List<TableArgumentAnalysis>> copartitioningLists = analyzeCopartitioning(node.getCopartitioning(), argumentsAnalysis.getTableArgumentAnalyses());
            ...
            if (returnTypeSpecification == ONLY_PASS_THROUGH) {
                ...
                if (!analysis.isAliased(node)) {
                    // spec p. 409
                    throw semanticException(INVALID_TABLE_FUNCTION_INVOCATION, node, "Alias missing for table function with GENERIC TABLE return type");
                }
                ...
                if (!analysis.isAliased(node)) {
                    // spec p. 409
                    throw semanticException(INVALID_TABLE_FUNCTION_INVOCATION, node, "Alias missing for table function with statically declared return type");
                }
        ...
        private List<List<TableArgumentAnalysis>> analyzeCopartitioning(List<List<QualifiedName>> copartitioning, List<TableArgumentAnalysis> tableArgumentAnalyses)
        {
            ...
            ImmutableList.Builder<List<TableArgumentAnalysis>> copartitionBuilder = ImmutableList.builder();
                ...
                for (int index = 0; index < partitioningColumns.get(0).size(); index++) {
                    ...
                    for (List<Expression> columnList : partitioningColumns) {
                        ...
                        if (!type.equals(commonSuperType)) {
                            ...
                            analysis.setCopartitionCoercion(column, commonSuperType);
                        }
                    }
                }

                // record the resolved copartition arguments
                copartitionBuilder.add(copartitionList);
            }
        ...
        protected Scope visitPatternRecognitionRelation(PatternRecognitionRelation relation, Optional<Scope> scope)
        {
            ...
            // MATCH_RECOGNIZE cannot be applied to a polymorphic table function (spec p.409)
        ...
        protected Scope visitAliasedRelation(AliasedRelation relation, Optional<Scope> scope)
        {
            ...
            if (relation.getRelation() instanceof TableFunctionInvocation tableFunctionInvocation) {
                return createAndAssignScope(relation, scope, aliasTableFunctionInvocation(relation, relationType, tableFunctionInvocation));
            }
        ...
        // As described by the spec p.409
        private RelationType aliasTableFunctionInvocation(AliasedRelation relation, RelationType relationType, TableFunctionInvocation tableFunctionInvocation)
        {
            TableFunctionInvocationAnalysis tableFunctionAnalysis = analysis.getTableFunctionAnalysis(tableFunctionInvocation);
            ...
            if (relation.getColumnNames() != null) {
                ...
                for (int i = 0; i < properColumnsCount; i++) {
                    ...
                    Optional<String> columnAlias = Optional.of(relation.getColumnNames().get(i).getCanonicalValue());
        ...
        protected Scope visitSampledRelation(SampledRelation relation, Optional<Scope> scope)
        {
            ...
            // TABLESAMPLE cannot be applied to a polymorphic table function (spec p.409)
