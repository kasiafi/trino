...
class StatementAnalyzer
{
            ...
            extends AstVisitor<Scope, Optional<Scope>>
    {
        ...
        protected Scope visitJsonTable(JsonTable node, Optional<Scope> scope)
        {
            ...
            CatalogHandle catalogHandle = getRequiredCatalogHandle(metadata, session, node, "system");
            ConnectorTransactionHandle transactionHandle = transactionManager.getConnectorTransaction(session.getRequiredTransactionId(), catalogHandle);
            analysis.addJsonTableAnalysis(node, new JsonTableAnalysis(catalogHandle, transactionHandle, parametersRowType));
        ...
        private void analyzeJsonTableColumns(List<JsonTableColumnDefinition> columns, Set<String> uniqueNames, ImmutableList.Builder<Field> outputFields, Scope enclosingScope, JsonTable jsonTable)
        {
