...
class StatementAnalyzer
{
            ...
            extends AstVisitor<Scope, Optional<Scope>>
    {
        ...
        protected Scope visitJsonTable(JsonTable node, Optional<Scope> scope)
        {
            ...
            Type parametersRowType = analyzeJsonPathInvocation(node, enclosingScope);
            ...
            analysis.addJsonTableAnalysis(node, new JsonTableAnalysis(catalogHandle, transactionHandle, parametersRowType, orderedOutputColumns.build()));
        ...
        private Type analyzeJsonPathInvocation(JsonTable node, Scope scope)
        {
            verifyNoAggregateWindowOrGroupingFunctions(session, metadata, node.getJsonPathInvocation().getInputExpression(), "JSON_TABLE input expression");
            node.getJsonPathInvocation().getPathParameters().stream()
                    .map(JsonPathParameter::getParameter)
                    .forEach(parameter -> verifyNoAggregateWindowOrGroupingFunctions(session, metadata, parameter, "JSON_TABLE path parameter"));
            ...
            return parametersTypeAndAnalysis.parametersRowType();
        }
                ...
                JsonTable jsonTable)
        {
            for (JsonTableColumnDefinition column : columns) {
                if (column instanceof OrdinalityColumn ordinalityColumn) {
                    ...
                    valueColumn.getEmptyDefault().ifPresent(expression -> verifyNoAggregateWindowOrGroupingFunctions(session, metadata, expression, "default expression for JSON_TABLE column"));
